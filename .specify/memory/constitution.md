<!--
同步影响报告
- 版本变更: 3.0.0 → 3.1.0 (MINOR: 简化目录结构约定，更新实验隔离描述)
- 变更内容:
  - 简化目录结构约定：仅保留必要的顶层目录，删除详细子目录结构
  - 更新实验隔离描述：从"Docker"改为"沙箱执行（当前 subprocess + venv，计划升级为 Docker）"
  - 变更动机：避免宪法锁定实现细节，保持技术栈描述与实际状态一致
- 历史版本:
  - 3.0.0 (2026-02-27): 删除可观测性和简单优先原则，新增 Python 环境管理规范，全文档中文化
  - 2.0.0: 删除 Observability、Simplicity & YAGNI 原则
-->
...
- 实验隔离：沙箱执行（当前 subprocess + venv，计划升级为 Docker）
...
**版本**: 3.1.0 | **生效日期**: 2026-03-02 | **最近修订**: 2026-03-02
